...
class LoaderService(object):
    ...
    def write(self, write_policy, meta, target_table, records, upsert_key=None, delete_keys=None, chunk_size=None):
        _log.info(f'Writing in {target_table} using {write_policy} strategy ...')
    ...
    def _compute_transformation(self, t, param_value=None):
        ...
        try:
            ...
            _log.error('Something went wrong while creating the underlying python function')
        ...
        if t['type'] == 'fit' and t['process_date'] is None:
            _log.info('Transformation has been set as \"fit\" kind. This must be processed !')
            try:
                ...
                _log.error('Something went wrong while deleting and inserting the result')
            ...
            _log.info('Transformation has been set as materialized \"tranform\" or \"predict\" kind. This must be processed !')
            try:
                if t['parameters'] is None:
                    ...
                    if len(t['parameters']) > 1:
                        _log.error('Multiple parameters has been provided which is currently not supported !')
                        raise BadRequest('Does not support transformation with multiple parameters')
                    param_name = t['parameters'][0]
                    if param_value is None:
                        _log.error('The transformation requires a parameter which looks empty or missing !')
                        raise BadRequest('Transformation requires a parameter')
                    _log.info('No parameters. We will delete the previous result according to the provided parameter')
